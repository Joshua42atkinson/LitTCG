...
fn handle_pet_taming_inputs(
    ...
    for (avatar, mut faces, element) in &mut pet_query {
        if keys.just_pressed(KeyCode::KeyP) {
            ...
            commands.spawn((
                AudioPlayer::<AudioSource>(asset_server.load(crate::asset_catalog::SOUND_PET)),
            ...
            chat_log.add_message(&avatar.word, &dialogue, t);
            commands.spawn((
                AudioPlayer::<AudioSource>(asset_server.load(crate::asset_catalog::SOUND_FEED)),
                PlaybackSettings::DESPAWN.with_spatial(true),
                Transform::from_xyz(0.0, 1.0, -2.0),
            ));
            speak_dialogue(dialogue, "af_bella".to_string(), &mut commands, &asset_server);
...
pub fn speak_dialogue(text: String, voice: String, commands: &mut Commands, asset_server: &Res<AssetServer>) {
    #[cfg(all(feature = "tts", not(target_arch = "wasm32")))]
    {
        ...
        let _ = commands;
        let _ = asset_server;
        crate::bridge::tts_client::request_tts_audio(text, voice);
    }
    ...
    #[cfg(any(not(feature = "tts"), target_arch = "wasm32"))]
    {
        ...
        let _ = voice;
        commands.spawn((
            AudioPlayer::<AudioSource>(asset_server.load(crate::asset_catalog::SOUND_BLIP)),
            PlaybackSettings::DESPAWN.with_spatial(true),
            Transform::from_xyz(0.0, 1.0, -2.0),
        ));
    }
...
pub fn trigger_social_combat(
    ...
    chat_log.add_message(&format!("SemanticSlime[{}]", word), &argument, time_elapsed);
...
mod tests {
    ...
    #[test]
    fn chat_log_adds_message_and_caps_at_five() {
        let mut log = ChatLog::default();
        for i in 0..7 {
            log.add_message("Barnaby", &format!("msg{}", i), i as f32);
        }
        assert_eq!(log.messages.len(), 5);
        assert_eq!(log.messages.first().unwrap().text, "msg2");
        assert_eq!(log.messages.last().unwrap().text, "msg6");
    }

    #[test]
    fn get_pet_dialogue_includes_element_word_and_description() {
        let text = get_pet_dialogue("clarity", Focus::Happy, Action::Playful, Element::Light);
        assert!(text.contains("Light"));
        assert!(text.contains("clarity"));
        assert!(text.contains("vibrates"));
    }

    #[test]
    fn get_pet_dialogue_fallback_for_unknown_focus_action() {
        let text = get_pet_dialogue("echo", Focus::Distant, Action::Hesitant, Element::Shadow);
        assert!(text.contains("Shadow"));
        assert!(text.contains("echo"));
        assert!(text.contains("glowing curiosity"));
    }
}
