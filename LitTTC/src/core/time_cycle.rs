...
mod tests {
    ...
    #[test]
    fn day_night_cycle_defaults_to_day() {
        let cycle = DayNightCycle::default();
        assert_eq!(cycle.current_phase, TimeOfDay::Day);
        assert_eq!(cycle.phase_duration, 30.0);
        assert_eq!(cycle.time_elapsed, 0.0);
    }

    #[test]
    fn time_of_day_progresses_in_order() {
        assert_eq!(TimeOfDay::Dawn, TimeOfDay::Dawn);
        let mut phase = TimeOfDay::Dawn;
        phase = match phase {
            TimeOfDay::Dawn => TimeOfDay::Day,
            TimeOfDay::Day => TimeOfDay::Dusk,
            TimeOfDay::Dusk => TimeOfDay::Night,
        ...
        assert_eq!(phase, TimeOfDay::Dusk);
    }
}
