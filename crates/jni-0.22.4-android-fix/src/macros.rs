    ...
    ( $jnienv:expr, $version:tt, $name:ident $(, $args:expr )* ) => ({
        jni_call_no_post_check_ex!($jnienv, $version, $name $(, $args)*).and_then(|ret| {
            if $jnienv.exception_check() {
                // Diagnostic: print the pending Java exception to stderr (Android logcat)
                // and clear it to avoid a fatal abort before we can return the error.
                eprintln!("JNI JavaException detected after {}", stringify!($name));
                let _ = jni_call_no_post_check_ex!($jnienv, v1_1, ExceptionDescribe);
                let _ = jni_call_no_post_check_ex!($jnienv, v1_1, ExceptionClear);
                Err(crate::errors::Error::JavaException)
            } else {
                Ok(ret)
            }
            ...
            if $jnienv.exception_check() {
                ...
                $jnienv.with_local_frame(
                    ...
                    |$env_mut| -> $crate::errors::Result<_> {
                        let Some(e) = $env_mut.exception_occurred() else {
                            // The exception cleared between exception_check and
                            // exception_occurred (observed on some Android runtimes).
                            // Treat as success to avoid a non-recoverable panic.
                            return Ok(ret);
                        };
            ...
            $jnienv.with_local_frame(
                ...
                |$env_mut| -> $ret_ty {
                    let Some(e) = $env_mut.exception_occurred() else {
                        // The exception cleared between exception_check and
                        // exception_occurred (observed on some Android runtimes).
                        // Return the original result to avoid a non-recoverable panic.
                        return ret;
                    };
                    $env_mut.exception_clear();
                    __jni_match_exceptions!($env_mut, e, $($ex_handlers)*)
